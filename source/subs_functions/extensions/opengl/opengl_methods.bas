                        ...
                        ELSE
                            FOR i = 1 TO d
                                IF GL_DEFINES$(i) = value$ THEN
                                    d = d + 1
                                    GL_DEFINES$(d) = a2$
                                    GL_DEFINES_VALUE(d) = GL_DEFINES_VALUE(i)
                                    EXIT FOR
                                END IF
                            NEXT
                        END IF
...
got_define:
            ...
            IF RIGHT$(a$, 1) = ";" THEN
                ...
                IF is_func THEN
                    GL_COMMAND_SUBFUNC(c) = 1
                    IF GL_KIT THEN PRINT #hk, "FUNCTION ";
                ELSE
                    GL_COMMAND_SUBFUNC(c) = 2
                    IF GL_KIT THEN PRINT #hk, "SUB ";
                END IF

                proc_name$ = readchunk(a$, l$)

                GL_COMMAND_CN$(c) = "_" + proc_name$
                IF GL_KIT THEN PRINT #hk, proc_name$;
                GL_COMMAND_CALLNAME$(c) = proc_name$

                GL_COMMAND_RET(c) = 0
                IF is_func THEN
                    pointer = 0
                    IF RIGHT$(ret_type$, 1) = "*" THEN pointer = 1
                    IF pointer THEN
                        t$ = "_OFFSET"
                        s$ = "&&"
                        ctyp$ = "ptrszint"
                        GL_COMMAND_RET(c) = OFFSETTYPE - ISPOINTER
                        hd$ = hd$ + "ptrszint "
                        need_helper_function = 1
                    ELSE
                        t$ = gl2qb_type_convert(ret_type$, s$, typ, ctyp$)
                        GL_COMMAND_RET(c) = typ
                        hd$ = hd$ + ctyp$ + " "
                    END IF
                ...
                DO
                    ...
                    IF var_type$ = "" AND l$ = "(" THEN var_type$ = readchunk(a$, l$)
                    ...
                    GL_COMMAND_ARGS(c) = GL_COMMAND_ARGS(c) + 1
                    MID$(GL_COMMAND_ARG$(c), (GL_COMMAND_ARGS(c) - 1) * 4 + 1, 4) = arg$

                    letter$ = CHR$(96 + GL_COMMAND_ARGS(c))
...
no_arguments:
                ...
                IF need_helper_function THEN
                    GL_HELPER_CODE = GL_HELPER_CODE + h$
                    GL_COMMAND_CALLNAME$(c) = "call_" + proc_name$
                END IF

                IF proc_name$ = "glGetString" THEN
                    GL_COMMAND_RET(c) = STRINGTYPE
                    GL_COMMAND_CALLNAME$(c) = "(  char*  )" + RTRIM$(GL_COMMAND_CALLNAME$(c))
                END IF
            END IF
        END IF
...
SUB gl_include_content
    ...
    FOR d = 1 TO GL_DEFINES_LAST
        ...
        IF constlast > constmax THEN
            ...
            REDIM _PRESERVE constnamesymbol(constmax) AS STRING
            REDIM _PRESERVE consttype(constmax) AS LONG
    ...
    FOR c = 1 TO GL_COMMANDS_LAST
        ...
        clearid
        id.ccall = 1
        id.n = RTRIM$(GL_COMMAND_CN$(c))
        IF qbnexprefix_set = 1 THEN id.n = MID$(RTRIM$(GL_COMMAND_CN$(c)), 2)
        s = GL_COMMAND_SUBFUNC(c)
        id.subfunc = s
        id.callname = RTRIM$(GL_COMMAND_CALLNAME$(c))
        id.args = GL_COMMAND_ARGS(c)
        id.arg = GL_COMMAND_ARG$(c)
        id.ret = GL_COMMAND_RET(c)
        regid

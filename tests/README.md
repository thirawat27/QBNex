...
- The current non-DOS conformance suite covers arrays and bounds, control flow, `DATA`/`READ`/`RESTORE`, numeric operators, `ON ERROR`, `PRINT USING`, procedures/`DEF FN`, random record I/O with `FIELD`/`LSET`/`GET`/`PUT`, sequential file I/O, string intrinsics, and user-defined types.

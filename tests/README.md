...
- `shell_cli` now also keeps an explicit-mode regression matrix: `-e` must fail fast for undeclared variables in default run, `-c`, and `-x`, while explicit-clean representative fixtures such as `const_and_def_fn`, `fixed_length_lset_rset`, `logical_comparisons`, `static_state`, `type_conversions`, and `user_defined_types` still have to match canonical output through those same three paths.
- The current non-DOS conformance suite covers arrays and bounds, array lifecycle with `ERASE`/`REDIM`, `BYVAL`/`BYREF`, `CLEAR` with `FREEFILE` recovery, `COMMON SHARED`, `COMMON SHARED` through `$INCLUDE`, computed branching with `ON GOTO`/`ON GOSUB`, console `INPUT`/`LINE INPUT`, constants and `DEF FN`, control flow, `DATA`/`READ`/`RESTORE`, `DEFINT`/`DEFLNG` default-type coercion, fixed-length string `LSET`/`RSET` on UDT fields, graphics mode pixel parity across `SCREEN 1` through `SCREEN 13`, graphics viewport/window mapping through `VIEW`/`WINDOW`/`PMAP`/`POINT` and noninteractive `LOCATE`, logical/comparison operators, loop controls, `MID$` assignment, numeric operators, `ON PLAY`, `ON TIMER`, `ON ERROR`, `PRINT USING`, procedures/`DEF FN`, random record I/O with `FIELD`/`LSET`/`GET`/`PUT`, text-screen state through `WIDTH`/`TAB`/`SCREEN`/`CSRLIN`, sequential file I/O, advanced `SELECT CASE`, `DIM SHARED`, `STATIC`, string intrinsics, `SWAP`, `CLEAR`, system-memory compatibility through `DEF SEG`/`PEEK`/`POKE`/`BSAVE`/`BLOAD`/`OUT`/`INP`, built-in type conversions, and user-defined types.
- Shell and conformance regressions now treat ANSI cursor/color escapes as UI-only: when stdout is captured or piped, `LOCATE`, cursor visibility, and `COLOR` should update runtime state without polluting program stdout.

...
fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("cli_tool crate should live under the workspace root")
        .to_path_buf()
}
...
fn qb_compiles_supported_qb64_source_files() {
    ...
    assert!(
        ...
        repo_root().join("qb64").join("source").display()
    );
    ...
    assert!(
        labels
            .iter()
            .any(|path| path == "qb64\\source\\qb64.bas" || path == "qb64/source/qb64.bas"),
        "expected qb64/source/qb64.bas in the full source sweep, found: {labels:?}"
    );
    assert!(
        labels
            .iter()
...
fn qb_compiles_qb64_source_roots_discovered_from_include_graph() {
    ...
    assert!(
        !roots.is_empty(),
        "expected at least one QB64 source root under {}",
        repo_root().join("qb64").join("source").display()
    );
    ...
    assert!(
        root_labels
            .iter()
            .any(|path| path == "qb64\\source\\qb64.bas" || path == "qb64/source/qb64.bas"),
        "expected qb64/source/qb64.bas to be discovered as a root, found: {root_labels:?}"
    );
...
fn compiler_and_native_codegen_cover_shipped_graphics_examples() {
    for example in [
        read_fixture("test/fixtures/basic/test_graphics_advanced.bas"),
        read_fixture("test/fixtures/basic/test_graphics_getput.bas"),
        read_fixture("test/fixtures/basic/test_graphics_modules.bas"),
    ] {

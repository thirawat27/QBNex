...
fn copy_built_binary(
    ...
    let target_bin = build_dir.join("release").join(format!("{binary_name}.exe"));
    let dest = output_path.to_path_buf();
    if target_bin.exists() {
        fs::copy(&target_bin, &dest)?;
    } else {
        let target_bin = build_dir.join("release").join(binary_name);
        if target_bin.exists() {
            fs::copy(&target_bin, &dest)?;
        } else {
...
fn shared_cargo_target_dir(name: &str) -> Result<PathBuf> {
    let dir = workspace_root_path()?
        .join("target")
        .join("qbnex-cargo-cache")
        .join(name);
    fs::create_dir_all(&dir)?;
    Ok(dir)
}

fn unique_bundle_name(prefix: &str) -> String {
    let counter = TEMP_PATH_COUNTER.fetch_add(1, Ordering::Relaxed);
    let nonce = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|duration| duration.as_nanos())
...
const VM_RUNNER_BINARY_NAME: &str = "qbnex_vm_runner";
const VM_RUNNER_PAYLOAD_MAGIC: &str = "QBNEX_VM_PAYLOAD_V1";

fn shared_runner_source_dir(name: &str) -> Result<PathBuf> {
    let dir = workspace_root_path()?
        .join("target")
        .join("qbnex-runner-src")
        .join(name);
    fs::create_dir_all(&dir)?;
    Ok(dir)
}

fn cached_vm_runner_binary_path() -> Result<PathBuf> {
    let target_dir = shared_cargo_target_dir("vm-runner-cache")?;
    let binary = if cfg!(target_os = "windows") {
        target_dir
            .join("release")
            .join(format!("{VM_RUNNER_BINARY_NAME}.exe"))
    } else {
        target_dir.join("release").join(VM_RUNNER_BINARY_NAME)
    };
    Ok(binary)
}
...
[dependencies]
hal_layer = {{ path = "{hal_layer_path}" }}
syntax_tree = {{ path = "{syntax_tree_path}" }}
analyzer = {{ path = "{analyzer_path}" }}
vm_engine = {{ path = "{vm_engine_path}" }}
...
panic = "abort"
"#,
        binary_name = VM_RUNNER_BINARY_NAME,
        hal_layer_path = hal_layer_path,
        syntax_tree_path = syntax_tree_path,
        analyzer_path = analyzer_path,
        vm_engine_path = vm_engine_path
    );
    fs::write(runner_src_dir.join("Cargo.toml"), cargo_toml)?;
    write_windows_static_config(&runner_src_dir)?;

    let src_dir = runner_src_dir.join("src");
...
const PAYLOAD_MAGIC: &[u8] = b"{payload_magic}";
...
fn run() -> Result<(), Box<dyn std::error::Error>> {{
    ...
    let (frontend_name, source) = decode_payload(&bytes)?;
...
fn decode_payload(bytes: &[u8]) -> Result<(String, String), Box<dyn std::error::Error>> {{
    let footer_len = PAYLOAD_MAGIC.len() + 8;
    if bytes.len() < footer_len {{
        return Err("missing embedded QBNex payload".into());
    }}
    let len_offset = bytes.len() - 8;
    let payload_len = u64::from_le_bytes(bytes[len_offset..].try_into()?) as usize;
    let magic_offset = len_offset - PAYLOAD_MAGIC.len();
    if &bytes[magic_offset..len_offset] != PAYLOAD_MAGIC {{
        return Err("invalid embedded QBNex payload marker".into());
    }}
    let payload_start = magic_offset
        .checked_sub(payload_len)
        .ok_or("corrupt embedded payload length")?;
    let payload = &bytes[payload_start..magic_offset];
    if payload.len() < 4 {{
        return Err("embedded payload is truncated".into());
    }}
    let frontend_len = u32::from_le_bytes(payload[..4].try_into()?) as usize;
    if payload.len() < 4 + frontend_len {{
        return Err("embedded frontend metadata is truncated".into());
    }}
    let frontend_name = String::from_utf8(payload[4..4 + frontend_len].to_vec())?;
    let source = String::from_utf8(payload[4 + frontend_len..].to_vec())?;
    Ok((frontend_name, source))
}}
"#,
        payload_magic = VM_RUNNER_PAYLOAD_MAGIC
    );
    ...
    if !binary.exists() {
        return Err(anyhow::anyhow!(
            "cached VM runner binary was not produced at '{}'",
            clean_path_display(&binary)
        ));
    }
...
fn embed_vm_runner_payload(output_path: &Path, source: &str, frontend: FrontendKind) -> Result<()> {
    let frontend_bytes = frontend.name().as_bytes();
    let source_bytes = source.as_bytes();
    let mut payload = Vec::with_capacity(4 + frontend_bytes.len() + source_bytes.len());
    payload.extend_from_slice(&(frontend_bytes.len() as u32).to_le_bytes());
    payload.extend_from_slice(frontend_bytes);
    payload.extend_from_slice(source_bytes);

    let mut output_file = fs::OpenOptions::new().append(true).open(output_path)?;
    output_file.write_all(&payload)?;
    output_file.write_all(VM_RUNNER_PAYLOAD_MAGIC.as_bytes())?;
    output_file.write_all(&(payload.len() as u64).to_le_bytes())?;
    output_file.flush()?;
    Ok(())
}
